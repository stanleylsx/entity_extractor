...
class DefaultModelVocabResizer:
    ...
    def set_embeddings(cls, model, token_ids):
        ...
        old_word_embeddings = getattr(model, 'model').embeddings.word_embeddings
        ...
        getattr(model, 'model').embeddings.word_embeddings = pruned_word_embeddings
    ...
    def set_lm_head(cls, model, token_ids) -> bool:
        ...
        output_embedding_layer.bias.data = torch.index_select(
            output_embedding_layer.bias.data, 0,
            index=torch.LongTensor(token_ids).to(output_embedding_layer.weight.device))
        return True
...
def get_num_of_trms(model):
    layer_template_regex = r"encoder.layer\.(\d+)\."
