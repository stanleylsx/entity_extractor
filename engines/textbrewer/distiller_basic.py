...
class BasicDistiller(AbstractDistiller):
    ...
    def __init__(self,
        ...
        self.best_at_epoch = 0

    def save_and_callback(self, global_step, step, epoch, val_dataloader, callback):
        if self.rank != 0:
            torch.distributed.barrier()  # save and eval with single process
        else:
            self.logger.info(f'saving at global step {global_step}, epoch step {step} epoch {epoch + 1}')
            if self.local_rank == 0:
                torch.distributed.barrier()
        if callback is not None:
            measures = callback(model=self.model_S, val_loader=val_dataloader)
            if measures['f1'] > self.best_f1:
                self.best_f1 = measures['f1']
                self.unprocessed = 0
                self.best_at_epoch = epoch + 1
                core_model = self.model_S.module if hasattr(self.model_S, 'module') else self.model_S
                state_dict = core_model.state_dict()
                torch.save(state_dict, os.path.join(self.t_config.output_dir, self.t_config.model_name))
                self.logger.info('saved the new best model with f1: %.3f in epoch: %s' % (self.best_f1, epoch + 1))
            else:
                self.unprocessed += 1

            if self.is_early_stop:
                if self.unprocessed >= self.patient:
                    self.logger.info('early stopped, no progress obtained within {} epochs'.format(self.patient))
                    self.logger.info('overall best f1 is {} at {} epoch'.format(self.best_f1, self.best_at_epoch))
                    exit(0)
            self.model_S.train()
    ...
    def initialize_training(self, optimizer, scheduler_class, scheduler_args, scheduler):
        ...
        self.logger.debug("Optimizer param group: ")
        self.logger.debug(f"{[[s.shape for s in g['params']] for g in optimizer.param_groups]}")
    ...
    def train_with_num_steps(self, optimizer, scheduler, print_per_batch, dataloader, max_grad_norm,
                             num_steps, val_dataloader, callback, batch_postprocessor, **args):
        ...
        self.logger.info(f'Total training steps: {total_global_steps}')
        self.logger.info(f'Checkpoints(step): {checkpoints}')
        self.logger.info(('+' * 20) + 'distilling start' + ('+' * 20))
        global_step = 0
        writer_step = 0
        step = 0
        for batch in tqdm(cycle(dataloader)):
            ...
            if (step + 1) % self.t_config.gradient_accumulation_steps == 0:
                ...
                if step % print_per_batch == 0 and step != 0:
                    self.logger.info(f'global step: {global_step}, epoch step:{step}')
                if (global_step % ckpt_steps == 0) or global_step == total_global_steps:
                    self.save_and_callback(global_step, step, 0, val_dataloader, callback)
            if global_step >= total_global_steps:
                self.logger.info('Training finished')
                return

    def train_with_num_epochs(self, optimizer, scheduler, print_per_batch, dataloader, max_grad_norm,
                              num_epochs, val_dataloader, callback, batch_postprocessor, **args):
        ...
        self.logger.info(f'Training steps per epoch: {train_steps_per_epoch}')
        self.logger.info(f'Checkpoints(step): {checkpoints}')
        ...
        if self.d_config.is_caching_logits is True:
            self.logger.info("Caching batches and teacher's logits...")
        ...
        for current_epoch in range(num_epochs):
            ...
            self.logger.info(f'Length of current epoch in forward batch: {len(dataloader)}')
            step = 0
            for batch in tqdm(dataloader):
                ...
                if (step + 1) % self.t_config.gradient_accumulation_steps == 0:
                    ...
                    if (global_step % train_steps_per_epoch in checkpoints) \
                            and ((current_epoch + 1) %
                                 self.t_config.ckpt_epoch_frequency == 0 or current_epoch + 1 == num_epochs):
                        self.save_and_callback(global_step, step, current_epoch, val_dataloader, callback)

    def train(self, optimizer, dataloader, print_per_batch, val_dataloader, num_epochs=None, scheduler_class=None,
              scheduler_args=None, scheduler=None, max_grad_norm=-1.0, num_steps=None, callback=None,
              batch_postprocessor=None, **args):
        """
        ...
        if num_steps is not None:
            self.train_with_num_steps(optimizer, scheduler, print_per_batch, dataloader, max_grad_norm,
                                      num_steps, val_dataloader, callback, batch_postprocessor, **args)
        else:
            self.train_with_num_epochs(optimizer, scheduler, print_per_batch, dataloader, max_grad_norm,
                                       num_epochs, val_dataloader, callback, batch_postprocessor, **args)
